...
class TrainLoop:
    ...
    def run_step(self, style, skt_cond):
        self.forward_backward(style, skt_cond)
    ...
    def forward_backward(self, style, skt_cond):
        self.opt.zero_grad()

        style = style.to(dist_util.dev())
        skt_cond = skt_cond.to(dist_util.dev())

        t, weights = self.schedule_sampler.sample(style.shape[0], dist_util.dev())
        ...
        compute_losses = functools.partial(
            ...
            style,
            t,
            model_kwargs={'physic_cond': skt_cond, 'x_start': style},
        )
